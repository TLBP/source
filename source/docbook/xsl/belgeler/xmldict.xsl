...
<xsl:template match="wordcount">
  <xsl:choose>
    <xsl:when test="@lang='tr'">
      <xsl:value-of select="count(//part[@id='trtr']/section/simplesect)"/>
    </xsl:when>
    <xsl:when test="@lang='en'">
      <xsl:value-of select="count(//part[@id='entr']/section/simplesect)"/>
    </xsl:when>
    <xsl:otherwise>
      <xsl:value-of select="count(//book[@id='sozluk']/part/section/simplesect)"/>
    </xsl:otherwise>
  </xsl:choose>
</xsl:template>
...
<xsl:template match="see-also">
  ...
  <xsl:apply-templates mode="xmldict"/>
