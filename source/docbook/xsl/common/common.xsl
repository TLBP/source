...
<!-- ********************************************************************
     $Id: common.xsl,v 1.4 2003/07/18 23:45:23 nilgun Exp $
     ********************************************************************
  ...
  <xsl:choose>
    <xsl:when test="$object/@id">
      <xsl:call-template name="check.id.unique">
        <xsl:with-param name="linkend" select="$object/@id"/>
      </xsl:call-template>
      <xsl:value-of select="$object/@id"/>
    </xsl:when>
    <xsl:otherwise>
      <!-- belgeler.org customization -->
      <xsl:variable name="objid0">
        <xsl:text>id</xsl:text>
        <xsl:if test="($object/ancestor-or-self::production)">
          <xsl:number from="/" count="production" format="1"/>
        </xsl:if>
        <xsl:if test="($object/ancestor-or-self::qandaentry)">
          <xsl:number from="/" count="qandaentry" format="1"/>
        </xsl:if>
        <xsl:if test="($object/ancestor-or-self::formalpara)">
          <xsl:number from="/" count="formalpara" format="1"/>
        </xsl:if>
        <xsl:if test="($object/ancestor-or-self::simplesect)">
          <xsl:number from="/" count="simplesect" format="1"/>
        </xsl:if>
        <xsl:if test="($object/ancestor-or-self::section)">
          <xsl:number from="/" count="section" format="1"/>
        ...
        <xsl:if test="($object/ancestor-or-self::bridgehead)">
          <xsl:number from="/" count="bridgehead" format="1"/>
        </xsl:if>
        <xsl:if test="($object/ancestor-or-self::sect5)">
          <xsl:number from="/" count="sect5" format="1"/>
        </xsl:if>
        <xsl:if test="($object/ancestor-or-self::sect4)">
          <xsl:number from="/" count="sect4" format="1"/>
        </xsl:if>
        <xsl:if test="($object/ancestor-or-self::sect3)">
          <xsl:number from="/" count="sect3" format="1"/>
        </xsl:if>
        <xsl:if test="($object/ancestor-or-self::sect2)">
          <xsl:number from="/" count="sect2" format="1"/>
        </xsl:if>
        <xsl:if test="($object/ancestor-or-self::refsect3)">
          <xsl:number from="/" count="refsect3" format="1"/>
        </xsl:if>
        <xsl:if test="($object/ancestor-or-self::refsect2)">
          <xsl:number from="/" count="refsect2" format="1"/>
        </xsl:if>
        <xsl:if test="($object/ancestor-or-self::refsect1)">
          <xsl:number from="/" count="refsect1" format="1"/>
        </xsl:if>
      </xsl:variable>
      <xsl:variable name="objid">
        <xsl:value-of select="$objid0"/>
        <xsl:if test="string-length($objid0) &lt; 3">
          <xsl:if test="($object/ancestor::book)">
            <xsl:number from="/" count="book" format="1"/>
          </xsl:if>
          <xsl:if test="($object/ancestor::reference)">
            <xsl:number from="/" count="reference" format="1"/>
          </xsl:if>
          <xsl:if test="($object/ancestor::part)">
            <xsl:number from="/" count="reference" format="1"/>
          </xsl:if>
          <xsl:if test="($object/ancestor::chapter)">
            <xsl:number from="/" count="chapter" format="1"/>
          </xsl:if>
          <xsl:if test="($object/ancestor::article)">
            <xsl:number from="/" count="article" format="1"/>
          </xsl:if>
          <xsl:if test="($object/ancestor::glossary)">
            <xsl:number from="/" count="glossary" format="1"/>
          </xsl:if>
          <xsl:if test="($object/ancestor::colophon)">
            <xsl:number from="/" count="colophon" format="1"/>
          </xsl:if>
          <xsl:if test="($object/ancestor::bibliography)">
            <xsl:number from="/" count="bibliography" format="1"/>
          </xsl:if>
          <xsl:if test="($object/ancestor::index)">
            <xsl:number from="/" count="index" format="1"/>
          </xsl:if>
          <xsl:if test="($object/ancestor::preface)">
            <xsl:number from="/" count="preface" format="1"/>
          </xsl:if>
          <xsl:if test="($object/ancestor::appendix)">
            <xsl:number from="/" count="appendix" format="1"/>
          </xsl:if>
          <xsl:if test="($object/ancestor::setindex)">
            <xsl:number from="/" count="setindex" format="1"/>
          </xsl:if>
          <xsl:if test="($object/ancestor::dictionary)">
            <xsl:number from="/" count="dictionary" format="1"/>
          </xsl:if>
          <xsl:if test="($object/ancestor::refentry)">
            <xsl:number from="/" count="refentry" format="1"/>
          </xsl:if>
          <xsl:if test="($object/ancestor::sect1)">
            <xsl:number from="/" count="sect1" format="1"/>
          </xsl:if>
        </xsl:if>
      </xsl:variable>
...
<xsl:template name="check.id.unique">
  ...
  <xsl:if test="$linkend != ''">
    ...
    <xsl:if test="count($targets)>1">
      <xsl:message>
        <xsl:text>***Warning***: ID already defined: </xsl:text>
        <xsl:value-of select="$linkend"/>
        <xsl:text>.</xsl:text>
      </xsl:message>
    </xsl:if>
