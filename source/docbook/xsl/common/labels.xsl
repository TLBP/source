...
<xsl:template match="set" mode="label.markup">
  <xsl:if test="@label">
...
<xsl:template match="book" mode="label.markup">
  <xsl:choose>
    ...
    <xsl:when test="$book.autolabel != 0">
...
<xsl:template match="part/article" mode="label.markup">
  <xsl:number from="part" count="article" format="1" level="any"/>
</xsl:template>

<xsl:template match="article/appendix" mode="label.markup">
  <xsl:number from="article" count="appendix" format="A" level="any"/>
</xsl:template>
...
<xsl:template match="bibliography|glossary|index|setindex|dictionary" mode="label.markup">
  <xsl:if test="@label">
    <xsl:value-of select="@label"/>
  </xsl:if>
</xsl:template>
    ...
    <xsl:otherwise>
      <xsl:choose>
        <xsl:when test="count($pchap)>0">
          ...
          <xsl:number format="1" from="chapter|article" level="any"/>
        </xsl:when>
        <xsl:otherwise>
