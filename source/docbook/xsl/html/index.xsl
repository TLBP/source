...
<xsl:template match="indexterm">
  ...
  <xsl:if test="name(..) = 'funcprototype'">
    <xsl:message terminate="yes">
<xsl:text>
        *** Fatal Error ***
*** indexterm in funcprototype ***
</xsl:text>
<xsl:value-of select="name(&section;)"/> id: <xsl:value-of select="&section;/@id"/> title: <xsl:apply-templates select="&section;" mode="title.markup"/>
    </xsl:message>
  </xsl:if>
